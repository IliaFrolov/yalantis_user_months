                    ...
                    <List spacing={2}>
                        {usersMonths.map(month => (
                            <ListItem className='month' xm={12} xs={12} key={month.number} >
                                <BadgeColored color={colorMarker(month.users.length)} badgeContent={month.users.length}>
                                    <Button style={{ width: '100%' }} className={`${colorMarker(month.users.length)} month_box`}
